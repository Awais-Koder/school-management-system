...
class FirstYearStudentTestImporter extends Importer
{
    ...
    public function resolveRecord(): ?FirstYearStudentTest
    {
        TempRollNumbers::firstOrCreate([
            'roll_number' => $this->data['roll_number'],
            'test_id' => $this->data['test_id'],
        ]);
        return FirstYearStudentTest::firstOrNew([
            'roll_number' => $this->data['roll_number'],
            'test_id' => $this->data['test_id'],
    ...
    {
        $body = 'Your first year student test import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
